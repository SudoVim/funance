...
class TickerOHLC(object):
    ...
    def query(self):
        ...
        def bulk_upload(dates, data):
            for date, row in zip(dates, data):
                entry = {
                    c: e for c, e in zip(df.columns, row)
                }
    ...
    def encode_entry(entry):
        ...
        entry['_source']['doc']['date'] = \
            dateutil.parser.parse(entry['_source']['doc']['date'])
            ...
            [
                tuple(
                    itertools.chain(
                        ...
                        (e['_source']['doc']['processed'].get(f, None) for f in processed_features),
                    ),
                ) for e in self.daily_iter() if 'processed' in e['_source']['doc']
            ],
    ...
    def process_ticker(self, start_date=None):
        ...
        resamples = {}
        for resample_period in ['5D', '10D', '30D', '90D', '180D', '360D', '540D', '720D']:
            resample = {}
        ...
        for shift_period in [-5, -10, -20, -30, -50, -90, -180, -360, -540, -720]:
            df['adjusted_close_shift_%sd' % (-shift_period)] = \
                df['Adj Close'].shift(shift_period)
            df['volume_shift_%sd' % (-shift_period)] = \
                df['Volume'].shift(shift_period)

        def bulk_update(columns, values):
            for entry in values:
                processed_data = {}

                for column, value in zip(columns[len(raw_columns):], entry[len(raw_columns):]):
                    if math.isnan(value):
                        continue
    ...
    def latest_batch_pandas(cls, features, processed_features, start_date=None):
        ...
        arr = np.array(
            [
                ...
                ) for e in latest_batch[1] if 'processed' in e['_source']['doc']
            ],
        ...
        return pd.DataFrame(data=arr[['symbol'] + features + processed_features], index=arr["date"])
